import { Property, PropertyGroup } from '@/types/game';
...
export const getAllBoardSpaces = () => {
  const spaces = Array(BOARD_SPACES).fill(null).map((_, index) => {
    // Check if it's a property
    const property = properties.find(p => p.position === index);
    if (property) {
      return { position: index, type: 'property', property };
    }
    
    // Check if it's a special space
    const specialSpace = specialSpaces.find(s => s.position === index);
    if (specialSpace) {
      return { position: index, type: specialSpace.type, ...specialSpace };
    }
    
    // Should never happen if data is complete
    return { position: index, type: 'unknown', name: `Unknown (${index})` };
  });
